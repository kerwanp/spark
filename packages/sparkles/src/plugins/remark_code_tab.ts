...
import { generateCodeBlockTabs, parseCodeBlockAttributes } from '../utils/codeblock.js'
      ...
      if (node.type === 'mdxJsxFlowElement' && node.name && node.name in Types) {
        ...
        if (node.name === 'Tabs' && localParseMdx) {
          localParseMdx = node.attributes.every(
            (attribute: any) => attribute.type !== 'mdxJsxAttribute' || attribute.name !== 'items'
          )
        }
      }
...
function processTabValue(nodes: Code[]) {
  ...
  for (const [i, node] of nodes.entries()) {
    const name = (node.data?.tab as string) ?? `Tab ${i + 1}`
    const li = out.get(name as string) ?? []
    li.push(node)
    out.set(name, li)
  }
  return out
...
function createFragment(children: MdxJsxFlowElement['children']): MdxJsxFlowElement {
  return {
    type: 'mdxJsxFlowElement',
    name: null,
    attributes: [],
