...
import { generateCodeBlockTabs, type CodeBlockTabsOptions } from '../utils/codeblock.js'
...
export function remarkInstall({
  persist = false,
  packageManagers = [
    { command: (cmd) => (convert as any)(cmd, 'npm'), name: 'npm' },
    { command: (cmd) => (convert as any)(cmd, 'pnpm'), name: 'pnpm' },
    { command: (cmd) => (convert as any)(cmd, 'yarn'), name: 'yarn' },
    { command: (cmd) => (convert as any)(cmd, 'bun'), name: 'bun' },
  ],
